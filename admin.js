...
const DEFAULT_DATA={
  ...
  categories:[],products:[],extras:[],borders:[],promoWeek:{}
};
...
function loadAdmin(){
  ...
  renderCategories();renderProducts();renderExtras();renderBorders();renderPromoWeek();
}
...
function addCategory(){
  ...
  catName.value="";saveDB(d);renderCategories();
}
...
function addProduct(){
  ...
  if(f){
    ...
    r.onload=()=>{p.image=r.result;d.products.push(p);saveDB(d);renderProducts();};
    r.readAsDataURL(f);
  }else{d.products.push(p);saveDB(d);renderProducts();}
  prodName.value=prodDesc.value=priceP.value=priceM.value=priceG.value=prodFlavors.value=prodImage.value="";
}
function renderProducts(){
  const d=loadDB();
  productList.innerHTML=d.products.map((p,i)=>`
    <div class="row">
      <input value="${p.name}" onchange="editProduct(${p.id},this.value)">
...
function editProduct(id,v){const d=loadDB();d.products.find(p=>p.id===id).name=v;saveDB(d);}
...
function renderExtras(){const d=loadDB();extraList.innerHTML=d.extras.map(e=>`<div class="row">${e.name} - R$ ${e.price}<button onclick="deleteExtra(${e.id})">🗑</button></div>`).join("");}
...
function renderBorders(){const d=loadDB();borderList.innerHTML=d.borders.map(b=>`<div class="row">${b.name} - R$ ${b.price}<button onclick="deleteBorder(${b.id})">🗑</button></div>`).join("");}
